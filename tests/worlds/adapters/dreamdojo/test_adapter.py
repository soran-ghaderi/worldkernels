...
from worldkernels.worlds.adapters.dreamdojo import adapter as dd_adapter
from worldkernels.worlds.adapters.dreamdojo.adapter import (
    CKPT_DIRS,
    EXPERIMENTS,
    DreamDojoWorld,
)
...
class TestEstimateVram:
    ...
    def test_grows_with_resolution(self):
        w = DreamDojoWorld()
        s = w.estimate_vram_mb(WorldConfig(height=64, width=64, frames_per_step=1))
        l_ = w.estimate_vram_mb(WorldConfig(height=512, width=512, frames_per_step=1))
        assert l_ > s
